...
def select_resolution(distance):
    ...
    valid_resolutions = df[df['Min Edge Length km (Hex)'] > distance/2]

    # Select the resolution with the minimum edge length from the valid resolutions
    selected_resolution = valid_resolutions.loc[valid_resolutions['Min Edge Length km (Hex)'].idxmin(), 'resolution']
